...
- [x] Repoint all repo links from the old GitHub username (`sstoilovABLE`) to the new one (`stoilms`) across the whole repo — `content.opf` (`dc:identifier`, publisher/VCS/editor-homepage URLs), `colophon.xhtml`, `README.md`, and the doc files (`CLAUDE.md`/`AGENTS.md`/`TASKS.md`). `se lint` stays clean (exit 0); rebuilt all four v1.0 formats (compatible `.epub`, advanced `.epub`, Kobo `.kepub.epub`, Kindle `.azw3`).
